...
makeRecoNtuple::makeRecoNtuple(int ac, char** av)
{
  ...
  for(Long64_t entry=entryStart; entry<entryStop; entry++){
	...
	//edited by Dipak
  if ( evtPick->passPresel_ele || evtPick->passPresel_mu || evtPick->passPresel_tau || saveAllEntries) {
...
//added by Dipak
    ...
    for (int i_tau = 0; i_tau <_nTau; i_tau++){
      ...
      _tauPt.push_back(tree->tauPt_[tauIndex]);
      _tauPhi.push_back(tree->tauPhi_[tauIndex]);
      _tauEta.push_back(tree->tauEta_[tauIndex]);
      lepCharge=tree->tauCharge_[tauIndex];
  ...
  int tauIndex_1 = selector->Taus.at(i_tau);
    _tauPt1.push_back(tree->tauPt_[tauIndex_1]);
    _tauPhi1.push_back(tree->tauPhi_[tauIndex_1]);
    _tauEta1.push_back(tree->tauEta_[tauIndex_1]);
    
    
    
    lepVector.SetPtEtaPhiM(tree->tauPt_[tauIndex_1],
    ...
    for(int j_tau = i_tau+1 ; j_tau <_nTau; j_tau++){
      ...
      _tauPt2.push_back(tree->tauPt_[tauIndex_2]);
      _tauPhi2.push_back(tree->tauPhi_[tauIndex_2]);
      _tauEta2.push_back(tree->tauEta_[tauIndex_2]);
	...
	sortDiTauInfo.index1_ = i_tau; 
	sortDiTauInfo.index2_ = j_tau;
 ...
 std::sort(sortDiTauInfos.begin(), sortDiTauInfos.end(), SortDiTauPairs()); 
  ...
  //  std::vector<DiTauInfo>sortedDiTaus; sortedDiTaus.clear(); 
  ...
  sortedDiTaus= sortDiTauInfos.at(0);
    ...
    float ditau_charge = sortedDiTaus.diTauCharge_;
