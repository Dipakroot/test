...
void Selector_gen::filter_taus(){
  for(int tauInd = 0; tauInd < tree->tauGenPartFlav_; ++tauInd){
    if  (tree->tauGenPartFlav_->at(tauInd)==5){
      double eta = tree->mcEta->at(tauInd);
      double pt = tree->mcPt->at(tauInd);
