...
void EventPick::process_event(EventTree* tree, Selector* selector, double weight){
	...
	//added by Dipak
	Pass_trigger_tau = Pass_trigger_tau && filters;
    ...
    if( passPresel_tau &&  Pass_trigger_tau) {
      if (saveCutflows) {cutFlow_mu->Fill(1); cutFlowWeight_mu->Fill(1,weight); dump_trigger_mu << run_lumi_event;} //have to change for saveCutflows
    }
    else { passPresel_tau = false;}
    if( passPresel_tau && isPVGood) { if (saveCutflows) {cutFlow_mu->Fill(2); cutFlowWeight_mu->Fill(2,weight); } }
    else { passPresel_tau = false;}
    ...
    //if ( passPresel_ele || passPresel_mu || passPresel_tau) {
    if (passPresel_tau) {  
    ...
    //added by Dipak
    // Cut on events with ==1 tau, no loose taus, no loose or tight electrons/muons                                                                                                 
    if (!QCDselect){
      if( passPresel_tau && selector->Taus.size() == Ntau_eq){
	if (Ntau_eq==2) {
	  int idx_tau1 = selector->Taus.at(0);
	  int idx_tau2 = selector->Taus.at(1);
	  if(tree->tauCharge_[idx_tau1]*tree->tauCharge_[idx_tau2] ==1){
	    passPresel_tau = false;
	  }
	  TLorentzVector tau1;
	  TLorentzVector tau2;
	  tau1.SetPtEtaPhiM(tree->tauPt_[idx_tau1],
			   tree->tauEta_[idx_tau1],
			   tree->tauPhi_[idx_tau1],
			   tree->tauMass_[idx_tau1]);
	  tau2.SetPtEtaPhiM(tree->tauPt_[idx_tau2],
			   tree->tauEta_[idx_tau2],
			   tree->tauPhi_[idx_tau2],
			   tree->tauMass_[idx_tau2]);
	  if (tree->event_==printEvent){
	    cout << "DilepMass:    " << (tau1 + tau2).M() << endl;
	    cout << "Lep 1 Charge: " << tree->tauCharge_[idx_tau1] << endl;
	    cout << "Lep 2 Charge: " << tree->tauCharge_[idx_tau2] << endl;
	    cout << "-------------------"<< endl;
	  }

	  if ( abs((tau1 + tau2).M() - 91.1876) > 10 ){
	    passPresel_tau = false;
	  }
	}
	if (saveCutflows){cutFlow_mu->Fill(3); cutFlowWeight_mu->Fill(3,weight); } //have to edit for saveCutflows
      }
      else { passPresel_tau = false;}
...
/* 
      ...
      if( passPresel_mu && (selector->ElectronsLoose.size() + selector->Electrons.size() ) <=  NlooseEleVeto_le ) {if (saveCutflows) {cutFlow_mu->Fill(5); cutFlowWeight_mu->\
																			     Fill(5,weight);  dump_lepton_mu << run_lumi_event;} }
      else { passPresel_mu = false;}
    }
    else{
